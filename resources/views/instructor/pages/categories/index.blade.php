<x-app-layout :title="'Categories'">
                                ...
                                <x-ui.td class="text-right whitespace-nowrap">
                                    <div class="flex justify-end gap-2">
                                        <button type="button"
                                            ...
                                            data-action="{{ route('instructor.categories.update', $cat->id) }}">
                                            <x-ui.icon name="pencil" class="w-4 h-4" />
                                        </button>

                                        <form action="{{ route('instructor.categories.destroy', $cat->id) }}" method="POST"
                                            class="inline" onsubmit="return confirm('Delete this category?')">
                                            @csrf @method('DELETE')
                                            <button type="submit"
                                                class="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete Category">
    ...
    <script>
        ...
        const inputName = modalEdit ? modalEdit.querySelector('#edit_name') : null;
        const inputSlug = modalEdit ? modalEdit.querySelector('#edit_slug') : null;
        const inputDesc = modalEdit ? modalEdit.querySelector('#edit_description') : null;
